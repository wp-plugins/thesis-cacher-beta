...
class WP_Object_Cache {
	...
	function add( $key, $data, $group = 'default', $expire = '' ) {
		...
		if ( $thesis_cache->cache_exists($key, $group) )
			return false;

		return $thesis_cache->set($key, $data, $group, $expire);
	}
	...
	function get( $key, $group = 'default', $force = false, &$found = null ) {
		...
		if (  $this->_exists( $key, $group ) ) {
			$found = true;
			$this->cache[$group][$key] = $thesis_cache->get($key,$group);
			$this->cache_hits += 1;
	...
	function set($key, $data, $group = 'default', $expire = '') {
		...
		$thesis_cache->set($key, $data, $group, $expire);
	...
	protected function _exists($key, $group) {
		global $thesis_cache;
		return $thesis_cache->cache_exists($key,$group);
	}
